<header class="bg-[url('/img/background_header.svg')] bg-cover bg-center h-[682px] flex justify-center items-center">
    <div class="flex flex-col items-center w-full max-w-screen-lg">

        <div class="h-[96px] w-[96px] border-2 border-solid border-[#E3646E] rounded-full items-center flex justify-center mb-[56px]">
            <img class="h-[87.27px] w-[87.27px] rounded-full object-cover border-2 border-solid rounded-[500px]" src="img/foto_perfil.jpg" alt="Foto de Ana Claudia Nogueira">
        </div>
        <div class="w-full max-w-[700px] text-center mx-auto mb-[80px] px-4">
            <p class="text-[#C0C4CE] font-[inconsolata] text-[16px] sm:text-[18px] md:text-[20px] leading-[140%] mb-[8px]">
                Hello World! Meu nome é <span class="text-[#E3646E]">Ana Claudia Nogueira</span> e sou
            </p>

            <h1 class="font-[asap] font-bold text-[32px] w-full sm:text-[40px] md:text-[48px] lg:text-[56px] leading-[120%] text-[#E2E4E9] mb-[20px]">
                Desenvolvedora PHP
            </h1>

            <p class="mx-auto font-[maven pro] text-[14px] sm:text-[16px] leading-[140%] text-[#878EA1] max-w-[700px]">
                Transformo necessidades em aplicações reais, envolventes e funcionais. Desenvolvo sistemas através da minha paixão pela tecnologia, contribuindo com soluções inovadoras e eficazes para desafios complexos.
            </p>
        </div>
        <div class="flex space-x-4">
            <div class="bg-[#82BC4F] text-black rounded-md px-2 py-1 font-semibold text-xs">GitHub</div>
            <div class="bg-[#BB72E9] text-black rounded-md px-2 py-1 font-semibold text-xs">PHP</div>
            <div class="bg-[#3996DB] text-black rounded-md px-2 py-1 font-semibold text-xs">CSS</div>
            <div class="bg-[#E3646E] text-black rounded-md px-2 py-1 font-semibold text-xs">HTML</div>
            <div class="bg-[#EABD5F] text-black rounded-md px-2 py-1 font-semibold text-xs">Javascript</div>
        </div>
    </div>
</header>