<?php
  ...
  $somaNotas = array_reduce($avaliacoes, function($carry, $a) {
    return ($carry ?? 0) + $a->nota;
  }) ?? 0;

  $mediaNotas = round($somaNotas / $totalAvaliacoes);
...
<div class="p-2 rounded border-stone-800 border-2 bg-stone-900">
    <div class="flex">
        ...
        <div class="space-y-1">
            ...
            <div class="text-xs italic"><?=  $notaFinal ?> (<?= $totalAvaliacoes  ?> Avaliações)</div>
        </div>
    </div>
...
<div class="grid grid-cols-4 gap-4">
    ...
    <div>
        ...
        <form action="/avaliar" method="post" class="p-4 space-y-4">
            ...
            <button type="submit" class="border-stone-800 bg-stone-900 text-stone-400 px-4 py-1 rounded-md border-2 hover:bg-stone-700">Enviar</button>
        </form>       
    </div>
     <?php endif; ?>
</div>